...
-- ADD CONSTRAINT fk_sucursal FOREIGN KEY (cod_sucursal) REFERENCES Sucursal4(cod_sucursal);
...
BEFORE INSERT OR UPDATE ON Sucursal4
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
--FOREIGN KEY (cod_director) REFERENCES Empleado4(cod_empleado) en Sucursal
...
BEFORE INSERT OR UPDATE ON Cliente4
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
-- FOREIGN KEY (cod_productor) REFERENCES Productor(cod_productor) en Vino4
...
BEFORE INSERT OR UPDATE ON SucursalVendeVino4
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
BEFORE INSERT OR UPDATE ON ClienteSolicitaSuministro4
FOR EACH ROW
BEGIN
