...
-- ADD CONSTRAINT fk_sucursal FOREIGN KEY (cod_sucursal) REFERENCES Sucursal2(cod_sucursal);
...
BEFORE INSERT OR UPDATE ON Sucursal2
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
--FOREIGN KEY (cod_director) REFERENCES Empleado2(cod_empleado) en Sucursal
...
BEFORE INSERT OR UPDATE ON Cliente2
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
-- FOREIGN KEY (cod_productor) REFERENCES Productor(cod_productor) en Vino2
...
BEFORE INSERT OR UPDATE ON SucursalVendeVino2
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
BEFORE INSERT OR UPDATE ON ClienteSolicitaSuministro2
FOR EACH ROW
BEGIN
