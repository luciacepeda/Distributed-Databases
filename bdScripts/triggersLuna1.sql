...
-- ADD CONSTRAINT fk_sucursal FOREIGN KEY (cod_sucursal) REFERENCES Sucursal1(cod_sucursal);
...
BEFORE INSERT OR UPDATE ON Sucursal1
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
--FOREIGN KEY (cod_director) REFERENCES Empleado1(cod_empleado) en Sucursal
...
BEFORE INSERT OR UPDATE ON Cliente1
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
BEGIN
    IF INSERTING THEN
        ...
        IF cont > 0 THEN
            RAISE_APPLICATION_ERROR(-20035,'Ya existe un productor con esa clave.');
        END IF;
    END IF;

    IF UPDATING('cod_productor') THEN
        RAISE_APPLICATION_ERROR(-20040,'No se puede cambiar el código de un productor.');
    END IF;
END;
/
...
-- FOREIGN KEY (cod_productor) REFERENCES Productor(cod_productor) en Vino1
...
BEFORE INSERT OR UPDATE ON SucursalVendeVino1
FOR EACH ROW
DECLARE cont NUMBER(3);
BEGIN
    IF INSERTING THEN
...
BEFORE INSERT OR UPDATE ON ClienteSolicitaSuministro1
FOR EACH ROW
BEGIN
...
BEGIN
    ...
    IF UPDATING('cod_sucursal_solicita')
       ...
       OR UPDATING('fecha_pedido') THEN
        RAISE_APPLICATION_ERROR(-20094,'No se puede cambiar la clave de la relación.');
    END IF;
END;
/
------------------------------------------------------------
...
BEGIN
    ...
    IF INSERTING OR UPDATING('cod_sucursal_solicita') THEN
        SELECT COUNT(*) INTO cont FROM Sucursal WHERE cod_sucursal = :NEW.cod_sucursal_solicita;
        IF cont = 0 THEN 
            RAISE_APPLICATION_ERROR(-20172,'La sucursal que solicita el pedido debe existir.');
        END IF;
    END IF;
    IF INSERTING OR UPDATING('cod_sucursal_provee') THEN    
        SELECT COUNT(*) INTO cont FROM Sucursal WHERE cod_sucursal = :NEW.cod_sucursal_provee;
        IF cont = 0 THEN 
            RAISE_APPLICATION_ERROR(-20173,'La sucursal que provee el pedido debe existir.');
        END IF;
    END IF;
